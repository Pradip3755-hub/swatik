...
def validate_checkin(doc, method):
    validate_employee(doc, method)
    prevent_multiple_checkins(doc, method)
    auto_checkout_missing(doc,method)
    mark_attendance_based_on_hours(doc, method)
...
def prevent_multiple_checkins(doc, method):
    if doc.log_type == "IN":
...
def mark_attendance_based_on_hours(doc, method):
...
def auto_checkout_missing(doc, method):
